...
CT(function()
	...
	ESX.TriggerServerCallback('opod-stash:GetStashes', function(stashes)
		...
		Opod:CreateBlips()
		-- print(ESX.DumpTable(Opod.StashHouses))
	end)
	while true do
		if Opod.PlayerData == nil then
			Opod.PlayerData = ESX.GetPlayerData()
		end
			...
			for k, v in ipairs(Opod.StashHouses) do
				if v.footEntrance ~= nil and not IsPedInAnyVehicle(Opod.PlayerPed, 0) then
					...
					if dist <= 50.0 then
						...
						if dist <= 1.0 then
							Opod:ShowContextMenu(_U("interact_warehouse"))
...
RegisterCommand('buildStash', function(source, args)
	while Opod.StashHouses == nil do Citizen.Wait(0) end
	if IsAceAllowed(Config.BuilderSettings.BuildAce) or Config.BuilderSettings.WhitelistedBuilders[Opod.PlayerData.identifier] then
		local type = ''
		if args[1] == '1' then
		...
		while not IsControlJustReleased(0, 47) do
			Citizen.Wait(5)
			Opod:ShowContextMenu(_U('place_foot_entrance'))
		...
		while not IsControlJustReleased(0, 47) do
			Citizen.Wait(5)
			Opod:ShowContextMenu(_U('place_car_entrance'))
